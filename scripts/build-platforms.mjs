...
for(const platform of platforms.filter(item=>!selectedSlugs?.length||selectedSlugs.includes(item.slug))){
 ...
 const renderedTools=tools.map(name=>{
  if(allTools.has(name)) return allTools.get(name);
  const spec=customTools.find(item=>item.name===name);
  if(!spec) throw new Error('Unknown tool '+name);
  const properties=spec.params.map(param=>{
   const base=param.type==='integer'?'Type.Integer':param.type==='number'?'Type.Number':param.type==='boolean'?'Type.Boolean':'Type.String';
   const options={description:param.description,...(param.minimum===undefined?{}:{minimum:param.minimum}),...(param.maximum===undefined?{}:{maximum:param.maximum})};
   const schema=param.enum?`Type.Union([${param.enum.map(value=>`Type.Literal(${JSON.stringify(value)})`).join(',')}], ${JSON.stringify(options)})`:`${base}(${JSON.stringify(options)})`;
   return `${JSON.stringify(param.name)}: ${param.required?schema:`Type.Optional(${schema})`}`;
  }).join(',');
  return `tool({name:${JSON.stringify(name)},description:${JSON.stringify(spec.description)},parameters:Type.Object({${properties}}),async execute(params,config,context){return callCrawlora(config,context.signal,${JSON.stringify(spec.operation)},params as Record<string, unknown>);}})`;
 }).join(',\n');
 const entry=`// Generated by scripts/build-platforms.mjs; edit src/index.ts and scripts/platforms.mjs.\nimport { Type } from "typebox";\nimport { defineToolPlugin } from "openclaw/plugin-sdk/tool-plugin";\nimport { callCrawlora } from "./client.js";\nconst plugin = defineToolPlugin({\n id: ${JSON.stringify(id)},\n name: ${JSON.stringify(title+' by Crawlora')},\n description: ${JSON.stringify(summary)},\n configSchema: ${property('configSchema').initializer.getText(tree)},\n tools: (tool) => [${renderedTools}],\n});\nconst register = plugin.register;\nplugin.register = (api) => {\n const umbrella = api.config?.plugins?.entries?.crawlora;\n if (umbrella && umbrella.enabled !== false) {\n  throw new Error("Disable the Crawlora starter plugin first (openclaw plugins disable crawlora). Its tools overlap with ${id}.");\n }\n return register(api);\n};\nexport default plugin;\n`;
