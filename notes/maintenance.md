...
### Amazon rescan and listing diagnosis

The live canonical URL `/crawlora-org/plugins/amazon` shows `AMBIGUOUS_SKILL_SLUG` for unrelated `amazon` skills owned by `ivangdavila` and `gecho-ai`. ClawHub's publisher plugin route first resolves `@crawlora-org/amazon`; because staged releases are not publicly visible, it then falls back to bare `amazon` through the unified package/skill API. That fallback throws before the route can check the returned owner. This is a ClawHub routing defect, not a collision in our runtime id or package scope.

The current ClawHub source exposes an owner-authorized `POST /api/v1/packages/{name}/rescan` with JSON `{ "version": "1.0.0" }` (`convex/httpApiV1/packagesV1.ts` and `convex/securityScan.ts`). This was submitted for the existing Amazon release and returned success, `alreadyQueued: true`, job `v57avn2aed0hyj4aw6hqb66fjx8e5g2g`. The handler raises the existing job's priority to manual, advances eligible scheduling, and requests dispatch without duplicating it. The artifact was rechecked: eight files including `package/package.json`, correct name/version, SHA-256 matching the staged receipt. Publication remains gated by the registry.

Do not assume the separately documented publish-attempt recovery route supports this community release: current `assertRecoveryArtifact` requires an original trusted publishing token for `openclaw/openclaw`, in addition to a failed attempt. Check exact eligibility before any recovery; ordinary publisher rescanning is the applicable supported action used here. Do not repeatedly enqueue an unchanged scan or change the package name/version to work around registry failures.

### SEC v1.0.1 manifest compatibility fix

The SEC warning was `manifest-unknown-fields` for `categories` and `uiHints`. Replace the sensitive-key hint with the supported `configContracts.secretInputs.paths` declaration (`apiKey`, expected string), and omit the optional category declaration so ClawHub can classify it. This supersedes the earlier advice to retain those two fields for SEC: an explicit, versioned secret-input contract is available in the tested host and preserves API-key sensitivity without unsupported top-level metadata. Other platform releases are unchanged.
...
All six expansion packages are published at v1.0.0 with clean ClawHub checks and zero public validation findings. Each exact public artifact was digest-verified and installed in the same fresh OpenClaw profile, with the expected single tool loaded; the installed manifests still mark API-key configuration sensitive through the host secret-input contract. The matching public artifacts are attached to https://github.com/Crawlora-org/crawlora-openclaw-plugin/releases/tag/expansion-v1.0.0, and GitHub's recorded digests match ClawHub. Final per-package release and install receipts are in `notes/releases/expansion-1.0.0.json`.

## Amazon v1.0.1 manifest warning fix

Amazon v1.0.0 is now publicly available after the earlier registry scan delay. Its remaining `manifest-unknown-fields` warning is fixed using the same supported metadata migration as SEC: omit top-level `categories`/`uiHints`, declare `configContracts.secretInputs.paths` for `apiKey`, and let ClawHub classify the package. Both Amazon tools are unchanged. All 83 tests pass; the real OpenClaw validator passes and Plugin Inspector reports zero issues. The actual host configuration schema still marks the API key sensitive.

Amazon v1.0.1 is public with clean security checks and zero warnings/errors in ClawHub's live validation summary. Its exact public download and GitHub artifact share SHA-256 `5b5bca1e89b4e33ecf96622fdae23ab37d9b552da6e0b0d9bb21755778de2daf`. A fresh public install loaded both tools, the installed config contract kept the API key sensitive, and a live Amazon search returned real results. The original 1.0.0 publication attempt is confirmed published and superseded. The first-batch GitHub release now points to the fixed Amazon/SEC patch releases; the Amazon follow-up can be closed. Final receipt: `notes/releases/amazon-1.0.1.json`.

## Jobs v1.0.0 publication

The Jobs package (`@crawlora-org/jobs`) is public on ClawHub: https://clawhub.ai/crawlora-org/plugins/jobs. It exposes the remaining two jobs tools, ATS discovery and hiring signals, without duplicating the provider-specific Greenhouse/Lever/Ashby packages. ClawHub checks are clean and its public validation summary reports zero warnings/errors. A fresh public download (SHA-256 `5347592697ea4d54751d0a1e44aa2548072966fe211833a3e691d4cc5f004ce4d`) installed successfully and loaded both tools; a live ATS lookup succeeded. GitHub release: https://github.com/Crawlora-org/crawlora-openclaw-plugin/releases/tag/jobs-v1.0.0. Final receipt: `notes/releases/jobs-1.0.0.json`.
