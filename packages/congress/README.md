# Congress by Crawlora

Public Congress data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Congress or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/congress --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-congress` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-congress --runtime --json
```

## Example

“Retrieve public Congress data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_congress_report` | Use Crawlora to access public Congress data. |

## Parameters

These are the exact generated input schemas.

### crawlora_congress_report
