# Starbucks by Crawlora

Public Starbucks data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Starbucks or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/starbucks --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-starbucks` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-starbucks --runtime --json
```

## Example

“Retrieve public Starbucks data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_starbucks_menu` | Use Crawlora to access public Starbucks data. |

## Parameters

These are the exact generated input schemas.

### crawlora_starbucks_menu
