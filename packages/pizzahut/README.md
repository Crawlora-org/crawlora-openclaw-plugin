# Pizzahut by Crawlora

Public Pizzahut data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Pizzahut or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/pizzahut --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-pizzahut` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-pizzahut --runtime --json
```

## Example

“Retrieve public Pizzahut data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_pizzahut_menu` | Use Crawlora to access public Pizzahut data. |

## Parameters

These are the exact generated input schemas.

### crawlora_pizzahut_menu
