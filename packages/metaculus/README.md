# Metaculus by Crawlora

Public Metaculus data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Metaculus or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/metaculus --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-metaculus` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-metaculus --runtime --json
```

## Example

“Retrieve public Metaculus data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_metaculus_question` | Use Crawlora to access public Metaculus data. |

## Parameters

These are the exact generated input schemas.

### crawlora_metaculus_question
