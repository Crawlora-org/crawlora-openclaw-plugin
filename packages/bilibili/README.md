# Bilibili by Crawlora

Public Bilibili data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Bilibili or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/bilibili --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-bilibili` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-bilibili --runtime --json
```

## Example

“Retrieve public Bilibili data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_bilibili_anime_home` | Use Crawlora to access public Bilibili data. |

## Parameters

These are the exact generated input schemas.

### crawlora_bilibili_anime_home
