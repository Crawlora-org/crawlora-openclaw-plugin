...
IMDb public title search through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official IMDb or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/imdb --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-imdb` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-imdb --runtime --json
```

## Example

“Search IMDb for science-fiction films.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_imdb_search` | Search public IMDb titles and names. |

## Parameters

These are the exact generated input schemas.

### crawlora_imdb_search
...
```json
{
  ...
  "properties": {
    "query": {
      "type": "string",
      "description": "Title or name query."
    },
    "limit": {
      "type": "number",
