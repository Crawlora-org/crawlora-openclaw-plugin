...
Public Subway data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Subway or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/subway --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-subway` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-subway --runtime --json
```

## Example

“Retrieve public Subway data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_subway_nearby` | Use Crawlora to access public Subway data. |

## Parameters

These are the exact generated input schemas.

### crawlora_subway_nearby
