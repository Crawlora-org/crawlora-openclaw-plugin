# Sparkfun by Crawlora

Public Sparkfun data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Sparkfun or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/sparkfun --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-sparkfun` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-sparkfun --runtime --json
```

## Example

“Retrieve public Sparkfun data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_sparkfun_search` | Use Crawlora to access public Sparkfun data. |

## Parameters

These are the exact generated input schemas.

### crawlora_sparkfun_search
