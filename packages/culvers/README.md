...
Public Culvers data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Culvers or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/culvers --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-culvers` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-culvers --runtime --json
```

## Example

“Retrieve public Culvers data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_culvers_directory` | Use Crawlora to access public Culvers data. |

## Parameters

These are the exact generated input schemas.

### crawlora_culvers_directory
