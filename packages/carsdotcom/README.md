# Carsdotcom by Crawlora

Public Carsdotcom data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Carsdotcom or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/carsdotcom --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-carsdotcom` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-carsdotcom --runtime --json
```

## Example

“Retrieve public Carsdotcom data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_carsdotcom_search` | Use Crawlora to access public Carsdotcom data. |

## Parameters

These are the exact generated input schemas.

### crawlora_carsdotcom_search
...
```json
{
  ...
  "properties": {
    ...
    "stock_type": {
      ...
      "description": "stock_type parameter."
    },
    "page": {
      "type": "integer",
