...
Public Zara data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Zara or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/zara --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-zara` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-zara --runtime --json
```

## Example

“Retrieve public Zara data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_zara_category_products` | Use Crawlora to access public Zara data. |

## Parameters

These are the exact generated input schemas.

### crawlora_zara_category_products
