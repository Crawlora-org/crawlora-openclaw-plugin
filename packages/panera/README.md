...
Public Panera data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Panera or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/panera --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-panera` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-panera --runtime --json
```

## Example

“Retrieve public Panera data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_panera_cafe` | Use Crawlora to access public Panera data. |

## Parameters

These are the exact generated input schemas.

### crawlora_panera_cafe
