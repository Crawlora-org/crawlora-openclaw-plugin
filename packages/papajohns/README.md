# Papajohns by Crawlora

Public Papajohns data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Papajohns or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/papajohns --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-papajohns` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-papajohns --runtime --json
```

## Example

“Retrieve public Papajohns data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_papajohns_directory` | Use Crawlora to access public Papajohns data. |

## Parameters

These are the exact generated input schemas.

### crawlora_papajohns_directory
