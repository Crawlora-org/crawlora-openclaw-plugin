# Courtlistener by Crawlora

Public Courtlistener data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Courtlistener or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/courtlistener --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-courtlistener` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-courtlistener --runtime --json
```

## Example

“Retrieve public Courtlistener data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_courtlistener_search` | Use Crawlora to access public Courtlistener data. |

## Parameters

These are the exact generated input schemas.

### crawlora_courtlistener_search
