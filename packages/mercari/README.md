...
Public Mercari data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Mercari or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/mercari --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-mercari` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-mercari --runtime --json
```

## Example

“Retrieve public Mercari data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_mercari_item` | Use Crawlora to access public Mercari data. |

## Parameters

These are the exact generated input schemas.

### crawlora_mercari_item
