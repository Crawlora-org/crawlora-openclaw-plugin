# Shakeshack by Crawlora

Public Shakeshack data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Shakeshack or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/shakeshack --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-shakeshack` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-shakeshack --runtime --json
```

## Example

“Retrieve public Shakeshack data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_shakeshack_nearby` | Use Crawlora to access public Shakeshack data. |

## Parameters

These are the exact generated input schemas.

### crawlora_shakeshack_nearby
