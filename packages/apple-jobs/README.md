# Apple Jobs by Crawlora

Apple public job search through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Apple Jobs or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/apple-jobs --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-apple-jobs` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-apple-jobs --runtime --json
```

## Example

“Search Apple Jobs for machine-learning roles.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_apple_jobs_search` | Search public Apple job openings. |

## Parameters

These are the exact generated input schemas.

### crawlora_apple_jobs_search
...
```json
{
  ...
  "properties": {
    "q": {
      "type": "string",
      "description": "Role or keyword query."
    },
    "location": {
      "type": "string",
