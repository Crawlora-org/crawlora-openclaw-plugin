...
Public Ulta data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Ulta or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/ulta --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-ulta` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-ulta --runtime --json
```

## Example

“Retrieve public Ulta data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_ulta_product` | Use Crawlora to access public Ulta data. |

## Parameters

These are the exact generated input schemas.

### crawlora_ulta_product
