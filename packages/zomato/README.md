...
Public Zomato data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Zomato or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/zomato --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-zomato` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-zomato --runtime --json
```

## Example

“Retrieve public Zomato data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_zomato_restaurant` | Use Crawlora to access public Zomato data. |

## Parameters

These are the exact generated input schemas.

### crawlora_zomato_restaurant
