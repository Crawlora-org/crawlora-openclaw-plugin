# Instacart by Crawlora

Public Instacart data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Instacart or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/instacart --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-instacart` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-instacart --runtime --json
```

## Example

“Retrieve public Instacart data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_instacart_search` | Use Crawlora to access public Instacart data. |

## Parameters

These are the exact generated input schemas.

### crawlora_instacart_search
...
```json
{
  ...
  "properties": {
    "shop_id": {
      "type": "string",
      "description": "shop_id parameter."
    },
    "store_slug": {
      "type": "string",
      "description": "store_slug parameter."
    },
    "q": {
      "type": "string",
