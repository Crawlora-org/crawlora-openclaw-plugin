...
Public Sephora data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Sephora or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/sephora --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-sephora` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-sephora --runtime --json
```

## Example

“Retrieve public Sephora data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_sephora_product` | Use Crawlora to access public Sephora data. |

## Parameters

These are the exact generated input schemas.

### crawlora_sephora_product
