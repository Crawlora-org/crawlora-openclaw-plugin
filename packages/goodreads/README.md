# Goodreads by Crawlora

Public Goodreads data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Goodreads or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/goodreads --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-goodreads` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-goodreads --runtime --json
```

## Example

“Retrieve public Goodreads data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_goodreads_search` | Use Crawlora to access public Goodreads data. |

## Parameters

These are the exact generated input schemas.

### crawlora_goodreads_search
