# Rightmove by Crawlora

Public Rightmove data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Rightmove or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/rightmove --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-rightmove` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-rightmove --runtime --json
```

## Example

“Retrieve public Rightmove data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_rightmove_property` | Use Crawlora to access public Rightmove data. |

## Parameters

These are the exact generated input schemas.

### crawlora_rightmove_property
