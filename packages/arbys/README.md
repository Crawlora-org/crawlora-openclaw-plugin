...
Public Arbys data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Arbys or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/arbys --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-arbys` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-arbys --runtime --json
```

## Example

“Retrieve public Arbys data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_arbys_menu` | Use Crawlora to access public Arbys data. |
