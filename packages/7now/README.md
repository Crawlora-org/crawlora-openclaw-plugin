...
Public 7now data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official 7now or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/7now --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-7now` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-7now --runtime --json
```

## Example

“Retrieve public 7now data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_7now_catalog` | Use Crawlora to access public 7now data. |

## Parameters

These are the exact generated input schemas.

### crawlora_7now_catalog
...
```json
{
  ...
  "properties": {
    ...
    "lat": {
      "type": "number",
      "description": "lat parameter."
    },
    "lon": {
      "type": "number",
      "description": "lon parameter."
    },
    "skip": {
      "type": "integer",
