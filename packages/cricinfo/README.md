# Cricinfo by Crawlora

Public Cricinfo data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Cricinfo or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/cricinfo --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-cricinfo` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-cricinfo --runtime --json
```

## Example

“Retrieve public Cricinfo data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_cricinfo_calendar` | Use Crawlora to access public Cricinfo data. |

## Parameters

These are the exact generated input schemas.

### crawlora_cricinfo_calendar
