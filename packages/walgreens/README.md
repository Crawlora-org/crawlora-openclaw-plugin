# Walgreens by Crawlora

Public Walgreens data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Walgreens or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/walgreens --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-walgreens` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-walgreens --runtime --json
```

## Example

“Retrieve public Walgreens data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_walgreens_stores` | Use Crawlora to access public Walgreens data. |

## Parameters

These are the exact generated input schemas.

### crawlora_walgreens_stores
