# Wingstop by Crawlora

Public Wingstop data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Wingstop or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/wingstop --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-wingstop` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-wingstop --runtime --json
```

## Example

“Retrieve public Wingstop data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_wingstop_nearby` | Use Crawlora to access public Wingstop data. |

## Parameters

These are the exact generated input schemas.

### crawlora_wingstop_nearby
