# Doordash by Crawlora

Public Doordash data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Doordash or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/doordash --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-doordash` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-doordash --runtime --json
```

## Example

“Retrieve public Doordash data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_doordash_search` | Use Crawlora to access public Doordash data. |

## Parameters

These are the exact generated input schemas.

### crawlora_doordash_search
...
```json
{
  ...
  "properties": {
    ...
    "dashPassOnly": {
      "type": "boolean",
      "description": "dashPassOnly parameter."
    },
    "asapOnly": {
      "type": "boolean",
      "description": "asapOnly parameter."
    },
    "pickupOnly": {
      "type": "boolean",
      "description": "pickupOnly parameter."
    },
    "maxDistanceMiles": {
      "type": "number",
      "description": "maxDistanceMiles parameter."
    }
  }
}
```
