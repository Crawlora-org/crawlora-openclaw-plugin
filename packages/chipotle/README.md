# Chipotle by Crawlora

Public Chipotle data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Chipotle or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/chipotle --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-chipotle` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-chipotle --runtime --json
```

## Example

“Retrieve public Chipotle data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_chipotle_menu` | Use Crawlora to access public Chipotle data. |

## Parameters

These are the exact generated input schemas.

### crawlora_chipotle_menu
