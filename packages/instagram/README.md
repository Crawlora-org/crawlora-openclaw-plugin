# Instagram by Crawlora

Public Instagram data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Instagram or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/instagram --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-instagram` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-instagram --runtime --json
```

## Example

“Retrieve public Instagram data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_instagram_post` | Use Crawlora to access public Instagram data. |

## Parameters

These are the exact generated input schemas.

### crawlora_instagram_post
...
```json
{
  ...
  "properties": {
    ...
    "post_id": {
      "type": "string",
      "description": "post_id path parameter."
    }
  }
}
```
