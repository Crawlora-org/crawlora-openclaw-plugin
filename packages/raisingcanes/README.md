# Raisingcanes by Crawlora

Public Raisingcanes data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Raisingcanes or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/raisingcanes --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-raisingcanes` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-raisingcanes --runtime --json
```

## Example

“Retrieve public Raisingcanes data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_raisingcanes_nearby` | Use Crawlora to access public Raisingcanes data. |

## Parameters

These are the exact generated input schemas.

### crawlora_raisingcanes_nearby
