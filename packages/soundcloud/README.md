# Soundcloud by Crawlora

Public Soundcloud data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Soundcloud or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/soundcloud --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-soundcloud` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-soundcloud --runtime --json
```

## Example

“Retrieve public Soundcloud data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_soundcloud_search` | Use Crawlora to access public Soundcloud data. |

## Parameters

These are the exact generated input schemas.

### crawlora_soundcloud_search
