# Importyeti by Crawlora

Public Importyeti data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Importyeti or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/importyeti --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-importyeti` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-importyeti --runtime --json
```

## Example

“Retrieve public Importyeti data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_importyeti_search` | Use Crawlora to access public Importyeti data. |

## Parameters

These are the exact generated input schemas.

### crawlora_importyeti_search
