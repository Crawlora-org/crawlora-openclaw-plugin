...
Public Ikea data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Ikea or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/ikea --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-ikea` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-ikea --runtime --json
```

## Example

“Retrieve public Ikea data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_ikea_product` | Use Crawlora to access public Ikea data. |

## Parameters

These are the exact generated input schemas.

### crawlora_ikea_product
