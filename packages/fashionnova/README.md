# Fashionnova by Crawlora

Public Fashionnova data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Fashionnova or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/fashionnova --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-fashionnova` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-fashionnova --runtime --json
```

## Example

“Retrieve public Fashionnova data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_fashionnova_product` | Use Crawlora to access public Fashionnova data. |

## Parameters

These are the exact generated input schemas.

### crawlora_fashionnova_product
