...
Public Wolt data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Wolt or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/wolt --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-wolt` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-wolt --runtime --json
```

## Example

“Retrieve public Wolt data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_wolt_restaurant` | Use Crawlora to access public Wolt data. |

## Parameters

These are the exact generated input schemas.

### crawlora_wolt_restaurant
