# Kickstarter by Crawlora

Public Kickstarter data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Kickstarter or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/kickstarter --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-kickstarter` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-kickstarter --runtime --json
```

## Example

“Retrieve public Kickstarter data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_kickstarter_project` | Use Crawlora to access public Kickstarter data. |

## Parameters

These are the exact generated input schemas.

### crawlora_kickstarter_project
