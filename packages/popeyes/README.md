...
Public Popeyes data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Popeyes or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/popeyes --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-popeyes` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-popeyes --runtime --json
```

## Example

“Retrieve public Popeyes data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_popeyes_menu` | Use Crawlora to access public Popeyes data. |

## Parameters

These are the exact generated input schemas.

### crawlora_popeyes_menu
