...
Public Strava data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Strava or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/strava --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-strava` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-strava --runtime --json
```

## Example

“Retrieve public Strava data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_strava_route_detail` | Use Crawlora to access public Strava data. |

## Parameters

These are the exact generated input schemas.

### crawlora_strava_route_detail
