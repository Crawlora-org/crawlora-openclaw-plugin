...
Public Manga data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Manga or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/manga --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-manga` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-manga --runtime --json
```

## Example

“Retrieve public Manga data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_manga_search` | Use Crawlora to access public Manga data. |

## Parameters

These are the exact generated input schemas.

### crawlora_manga_search
