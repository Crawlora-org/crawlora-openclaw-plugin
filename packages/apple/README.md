...
Public Apple data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Apple or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/apple --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-apple` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-apple --runtime --json
```

## Example

“Retrieve public Apple data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_apple_books_audiobook_search` | Use Crawlora to access public Apple data. |

## Parameters

These are the exact generated input schemas.

### crawlora_apple_books_audiobook_search
