...
Public Threads data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Threads or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/threads --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-threads` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-threads --runtime --json
```

## Example

“Retrieve public Threads data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_threads_search` | Use Crawlora to access public Threads data. |

## Parameters

These are the exact generated input schemas.

### crawlora_threads_search
