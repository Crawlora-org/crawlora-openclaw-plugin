# Meta Jobs by Crawlora

Public Meta Jobs data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Meta Jobs or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/meta-jobs --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-meta-jobs` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-meta-jobs --runtime --json
```

## Example

“Retrieve public Meta Jobs data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_meta_jobs_search` | Use Crawlora to access public Meta Jobs data. |

## Parameters

These are the exact generated input schemas.

### crawlora_meta_jobs_search
...
```json
{
  ...
  "properties": {
    ...
    "offices": {
      "type": "string",
      "description": "offices parameter."
    },
    "is_remote_only": {
      "type": "boolean",
      "description": "is_remote_only parameter."
    },
    "sort_by_new": {
      "type": "boolean",
      "description": "sort_by_new parameter."
    },
    "results_per_page": {
      "anyOf": [
        {
      ...
      "description": "results_per_page parameter."
    }
  }
}
```
