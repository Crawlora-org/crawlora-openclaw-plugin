# Everlane by Crawlora

Public Everlane data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Everlane or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/everlane --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-everlane` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-everlane --runtime --json
```

## Example

“Retrieve public Everlane data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_everlane_product` | Use Crawlora to access public Everlane data. |

## Parameters

These are the exact generated input schemas.

### crawlora_everlane_product
