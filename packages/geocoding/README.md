# Geocoding by Crawlora

Public Geocoding data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Geocoding or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/geocoding --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-geocoding` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-geocoding --runtime --json
```

## Example

“Retrieve public Geocoding data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_geocoding_search` | Use Crawlora to access public Geocoding data. |

## Parameters

These are the exact generated input schemas.

### crawlora_geocoding_search
...
```json
{
  ...
  "properties": {
    ...
    "postalcode": {
      "type": "string",
      "description": "postalcode parameter."
    },
    "limit": {
      "type": "integer",
    ...
    "countrycodes": {
      "type": "string",
      "description": "countrycodes parameter."
    },
    "accept_language": {
      "type": "string",
      "description": "accept_language parameter."
    },
    "addressdetails": {
      "type": "boolean",
      "description": "addressdetails parameter."
    },
    "extratags": {
      "type": "boolean",
      "description": "extratags parameter."
    },
    "namedetails": {
      "type": "boolean",
      "description": "namedetails parameter."
    }
  }
}
```
