...
Public Rover data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Rover or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/rover --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-rover` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-rover --runtime --json
```

## Example

“Retrieve public Rover data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_rover_sitter_search` | Use Crawlora to access public Rover data. |

## Parameters

These are the exact generated input schemas.

### crawlora_rover_sitter_search
...
```json
{
  ...
  "properties": {
    ...
    "service_type": {
      "type": "string",
      "description": "service_type parameter."
    },
    "page": {
      "type": "integer",
    ...
    "pet_type": {
      "type": "string",
      "description": "pet_type parameter."
    },
    "min_price": {
      "type": "integer",
    ...
    "star_sitter_only": {
      "type": "boolean",
      "description": "star_sitter_only parameter."
    }
  }
}
```
