...
Public Discogs data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Discogs or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/discogs --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-discogs` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-discogs --runtime --json
```

## Example

“Retrieve public Discogs data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_discogs_search` | Use Crawlora to access public Discogs data. |

## Parameters

These are the exact generated input schemas.

### crawlora_discogs_search
