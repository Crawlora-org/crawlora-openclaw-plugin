# Pitchbook by Crawlora

Public Pitchbook data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Pitchbook or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/pitchbook --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-pitchbook` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-pitchbook --runtime --json
```

## Example

“Retrieve public Pitchbook data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_pitchbook_company` | Use Crawlora to access public Pitchbook data. |

## Parameters

These are the exact generated input schemas.

### crawlora_pitchbook_company
