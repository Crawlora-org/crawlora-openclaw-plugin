...
Public Patreon data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Patreon or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/patreon --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-patreon` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-patreon --runtime --json
```

## Example

“Retrieve public Patreon data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_patreon_creator` | Use Crawlora to access public Patreon data. |

## Parameters

These are the exact generated input schemas.

### crawlora_patreon_creator
