...
TikTok public post data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official TikTok or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/tiktok --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-tiktok` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-tiktok --runtime --json
```

## Example

“Fetch a public TikTok post by ID.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_tiktok_post` | Fetch a public TikTok post by ID. |

## Parameters

These are the exact generated input schemas.

### crawlora_tiktok_post
