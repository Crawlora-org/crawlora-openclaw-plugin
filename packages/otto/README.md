...
Public Otto data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Otto or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/otto --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-otto` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-otto --runtime --json
```

## Example

“Retrieve public Otto data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_otto_search` | Use Crawlora to access public Otto data. |

## Parameters

These are the exact generated input schemas.

### crawlora_otto_search
