...
Public Roblox data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Roblox or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/roblox --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-roblox` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-roblox --runtime --json
```

## Example

“Retrieve public Roblox data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_roblox_game` | Use Crawlora to access public Roblox data. |

## Parameters

These are the exact generated input schemas.

### crawlora_roblox_game
...
```json
{
  ...
  "properties": {
    "universe_id": {
      "type": "integer",
      "description": "universe_id parameter."
    }
  }
}
```
