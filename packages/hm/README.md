...
Public Hm data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Hm or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/hm --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-hm` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-hm --runtime --json
```

## Example

“Retrieve public Hm data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_hm_listing` | Use Crawlora to access public Hm data. |
  ...
  "properties": {
    ...
    "sort": {
      "anyOf": [
        ...
        {
          "type": "string",
          "const": "PRICE_ASCENDING"
        },
        {
          "type": "string",
          "const": "PRICE_DESCENDING"
        }
      ],
