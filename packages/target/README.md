...
Public Target data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Target or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/target --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-target` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-target --runtime --json
```

## Example

“Retrieve public Target data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_target_product` | Use Crawlora to access public Target data. |

## Parameters

These are the exact generated input schemas.

### crawlora_target_product
