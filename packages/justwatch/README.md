# Justwatch by Crawlora

Public Justwatch data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Justwatch or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/justwatch --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-justwatch` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-justwatch --runtime --json
```

## Example

“Retrieve public Justwatch data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_justwatch_discover` | Use Crawlora to access public Justwatch data. |

## Parameters

These are the exact generated input schemas.

### crawlora_justwatch_discover
...
```json
{
  ...
  "properties": {
    ...
    "production_countries": {
      "type": "string",
      "description": "production_countries parameter."
    },
    "monetization_types": {
      "anyOf": [
        {
      ...
      "description": "monetization_types parameter."
    },
    "year_min": {
      "type": "integer",
      "description": "year_min parameter."
    },
    "year_max": {
      "type": "integer",
      "description": "year_max parameter."
    }
  }
}
```
