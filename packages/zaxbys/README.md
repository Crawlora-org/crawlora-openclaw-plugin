...
Public Zaxbys data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Zaxbys or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/zaxbys --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-zaxbys` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-zaxbys --runtime --json
```

## Example

“Retrieve public Zaxbys data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_zaxbys_nearby` | Use Crawlora to access public Zaxbys data. |

## Parameters

These are the exact generated input schemas.

### crawlora_zaxbys_nearby
