# Tokopedia by Crawlora

Public Tokopedia data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Tokopedia or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/tokopedia --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-tokopedia` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-tokopedia --runtime --json
```

## Example

“Retrieve public Tokopedia data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_tokopedia_search` | Use Crawlora to access public Tokopedia data. |

## Parameters

These are the exact generated input schemas.

### crawlora_tokopedia_search
