...
OpenSea public NFT collection data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official OpenSea or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/opensea --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-opensea` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-opensea --runtime --json
```

## Example

“Inspect a public OpenSea NFT collection.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_opensea_collection` | Fetch public OpenSea collection data. |

## Parameters

These are the exact generated input schemas.

### crawlora_opensea_collection
