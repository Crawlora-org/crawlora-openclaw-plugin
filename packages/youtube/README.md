...
YouTube video transcripts through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official YouTube or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/youtube --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-youtube` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-youtube --runtime --json
```
