# Googleplay by Crawlora

Public Googleplay data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Googleplay or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/googleplay --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-googleplay` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-googleplay --runtime --json
```

## Example

“Retrieve public Googleplay data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_googleplay_app` | Use Crawlora to access public Googleplay data. |

## Parameters

These are the exact generated input schemas.

### crawlora_googleplay_app
