...
Public Skims data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Skims or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/skims --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-skims` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-skims --runtime --json
```

## Example

“Retrieve public Skims data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_skims_product` | Use Crawlora to access public Skims data. |

## Parameters

These are the exact generated input schemas.

### crawlora_skims_product
