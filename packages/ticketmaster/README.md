# Ticketmaster by Crawlora

Public Ticketmaster data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Ticketmaster or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/ticketmaster --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-ticketmaster` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-ticketmaster --runtime --json
```

## Example

“Retrieve public Ticketmaster data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_ticketmaster_attraction` | Use Crawlora to access public Ticketmaster data. |

## Parameters

These are the exact generated input schemas.

### crawlora_ticketmaster_attraction
