...
Public Chewy data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Chewy or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/chewy --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-chewy` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-chewy --runtime --json
```

## Example

“Retrieve public Chewy data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_chewy_product` | Use Crawlora to access public Chewy data. |

## Parameters

These are the exact generated input schemas.

### crawlora_chewy_product
