# Similarweb by Crawlora

Public Similarweb data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Similarweb or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/similarweb --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-similarweb` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-similarweb --runtime --json
```

## Example

“Retrieve public Similarweb data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_similarweb_search` | Use Crawlora to access public Similarweb data. |

## Parameters

These are the exact generated input schemas.

### crawlora_similarweb_search
