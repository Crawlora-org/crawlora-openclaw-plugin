# Trustmrr by Crawlora

Public Trustmrr data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Trustmrr or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/trustmrr --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-trustmrr` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-trustmrr --runtime --json
```

## Example

“Retrieve public Trustmrr data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_trustmrr_marketplace` | Use Crawlora to access public Trustmrr data. |

## Parameters

These are the exact generated input schemas.

### crawlora_trustmrr_marketplace
