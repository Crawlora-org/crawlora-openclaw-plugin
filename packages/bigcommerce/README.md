# Bigcommerce by Crawlora

Public Bigcommerce data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Bigcommerce or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/bigcommerce --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-bigcommerce` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-bigcommerce --runtime --json
```

## Example

“Retrieve public Bigcommerce data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_bigcommerce_search` | Use Crawlora to access public Bigcommerce data. |

## Parameters

These are the exact generated input schemas.

### crawlora_bigcommerce_search
