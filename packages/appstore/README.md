# Appstore by Crawlora

Public Appstore data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Appstore or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/appstore --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-appstore` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-appstore --runtime --json
```

## Example

“Retrieve public Appstore data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_appstore_search` | Use Crawlora to access public Appstore data. |

## Parameters

These are the exact generated input schemas.

### crawlora_appstore_search
...
```json
{
  ...
  "properties": {
    ...
    "ids_only": {
      "type": "boolean",
      "description": "ids_only parameter."
    },
    "platform": {
      "anyOf": [
        {
