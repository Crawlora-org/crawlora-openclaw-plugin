...
Public Bestbuy data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Bestbuy or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/bestbuy --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-bestbuy` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-bestbuy --runtime --json
```

## Example

“Retrieve public Bestbuy data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_bestbuy_product` | Use Crawlora to access public Bestbuy data. |

## Parameters

These are the exact generated input schemas.

### crawlora_bestbuy_product
