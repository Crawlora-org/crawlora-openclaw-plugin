...
Public Fiverr data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Fiverr or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/fiverr --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-fiverr` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-fiverr --runtime --json
```

## Example

“Retrieve public Fiverr data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_fiverr_search` | Use Crawlora to access public Fiverr data. |

## Parameters

These are the exact generated input schemas.

### crawlora_fiverr_search
