# Trustpilot by Crawlora

Public Trustpilot data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Trustpilot or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/trustpilot --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-trustpilot` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-trustpilot --runtime --json
```

## Example

“Retrieve public Trustpilot data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_trustpilot_business` | Use Crawlora to access public Trustpilot data. |

## Parameters

These are the exact generated input schemas.

### crawlora_trustpilot_business
