# Fiveguys by Crawlora

Public Fiveguys data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Fiveguys or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/fiveguys --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-fiveguys` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-fiveguys --runtime --json
```

## Example

“Retrieve public Fiveguys data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_fiveguys_nearby` | Use Crawlora to access public Fiveguys data. |

## Parameters

These are the exact generated input schemas.

### crawlora_fiveguys_nearby
