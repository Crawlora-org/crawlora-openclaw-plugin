...
Public Redfin data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Redfin or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/redfin --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-redfin` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-redfin --runtime --json
```

## Example

“Retrieve public Redfin data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_redfin_property` | Use Crawlora to access public Redfin data. |

## Parameters

These are the exact generated input schemas.

### crawlora_redfin_property
...
```json
{
  ...
  "properties": {
    ...
    "property_id": {
      "type": "string",
      "description": "property_id parameter."
    },
    "listing_id": {
      "type": "string",
