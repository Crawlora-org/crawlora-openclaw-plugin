...
Public Walmart data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Walmart or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/walmart --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-walmart` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-walmart --runtime --json
```

## Example

“Retrieve public Walmart data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_walmart_search` | Use Crawlora to access public Walmart data. |

## Parameters

These are the exact generated input schemas.

### crawlora_walmart_search
