...
Public Dominos data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Dominos or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/dominos --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-dominos` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-dominos --runtime --json
```

## Example

“Retrieve public Dominos data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_dominos_menu` | Use Crawlora to access public Dominos data. |

## Parameters

These are the exact generated input schemas.

### crawlora_dominos_menu
