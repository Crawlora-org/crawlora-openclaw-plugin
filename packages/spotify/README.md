...
Spotify catalog search through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Spotify or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/spotify --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-spotify` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-spotify --runtime --json
```

## Example

“Search Spotify for ambient focus music.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_spotify_search` | Search Spotify for tracks, artists, albums, playlists, and podcasts. |

## Parameters

These are the exact generated input schemas.

### crawlora_spotify_search
