...
Public Costco data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Costco or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/costco --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-costco` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-costco --runtime --json
```

## Example

“Retrieve public Costco data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_costco_product` | Use Crawlora to access public Costco data. |

## Parameters

These are the exact generated input schemas.

### crawlora_costco_product
