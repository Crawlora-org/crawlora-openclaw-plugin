...
Public Bbb data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Bbb or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/bbb --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-bbb` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-bbb --runtime --json
```

## Example

“Retrieve public Bbb data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_bbb_business` | Use Crawlora to access public Bbb data. |

## Parameters

These are the exact generated input schemas.

### crawlora_bbb_business
