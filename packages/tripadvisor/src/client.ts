...
export async function callCrawlora(
  ...
  if (!apiKey) {
    throw new Error("Crawlora API key missing. Set plugins.entries.crawlora-tripadvisor.config.apiKey or CRAWLORA_API_KEY.");
  }
  ...
  const client = new CrawloraClient({
    ...
    timeout: 60_000,
    retries: 0,
    userAgent: "crawlora-tripadvisor/1.0.0",
  });
  try {
