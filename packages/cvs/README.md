...
Public Cvs data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Cvs or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/cvs --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-cvs` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-cvs --runtime --json
```

## Example

“Retrieve public Cvs data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_cvs_search` | Use Crawlora to access public Cvs data. |
