# Duckduckgo by Crawlora

Public Duckduckgo data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Duckduckgo or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/duckduckgo --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-duckduckgo` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-duckduckgo --runtime --json
```

## Example

“Retrieve public Duckduckgo data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_duckduckgo_search` | Use Crawlora to access public Duckduckgo data. |

## Parameters

These are the exact generated input schemas.

### crawlora_duckduckgo_search
...
```json
{
  ...
  "properties": {
    ...
    "safe_search": {
      ...
      "description": "safe_search parameter."
    }
  }
}
```
