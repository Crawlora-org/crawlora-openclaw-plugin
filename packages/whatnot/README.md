...
Public Whatnot data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Whatnot or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/whatnot --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-whatnot` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-whatnot --runtime --json
```

## Example

“Retrieve public Whatnot data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_whatnot_browse` | Use Crawlora to access public Whatnot data. |

## Parameters

These are the exact generated input schemas.

### crawlora_whatnot_browse
