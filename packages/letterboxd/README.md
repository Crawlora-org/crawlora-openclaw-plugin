# Letterboxd by Crawlora

Public Letterboxd data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Letterboxd or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/letterboxd --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-letterboxd` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-letterboxd --runtime --json
```

## Example

“Retrieve public Letterboxd data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_letterboxd_search` | Use Crawlora to access public Letterboxd data. |

## Parameters

These are the exact generated input schemas.

### crawlora_letterboxd_search
