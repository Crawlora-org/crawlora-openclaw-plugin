...
Public Wayfair data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Wayfair or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/wayfair --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-wayfair` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-wayfair --runtime --json
```

## Example

“Retrieve public Wayfair data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_wayfair_product` | Use Crawlora to access public Wayfair data. |

## Parameters

These are the exact generated input schemas.

### crawlora_wayfair_product
