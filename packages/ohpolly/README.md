...
Public Ohpolly data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Ohpolly or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/ohpolly --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-ohpolly` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-ohpolly --runtime --json
```

## Example

“Retrieve public Ohpolly data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_ohpolly_product` | Use Crawlora to access public Ohpolly data. |

## Parameters

These are the exact generated input schemas.

### crawlora_ohpolly_product
