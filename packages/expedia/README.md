...
Public Expedia data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Expedia or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/expedia --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-expedia` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-expedia --runtime --json
```

## Example

“Retrieve public Expedia data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_expedia_activities_search` | Use Crawlora to access public Expedia data. |

## Parameters

These are the exact generated input schemas.

### crawlora_expedia_activities_search
