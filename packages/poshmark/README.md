# Poshmark by Crawlora

Public Poshmark data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Poshmark or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/poshmark --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-poshmark` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-poshmark --runtime --json
```

## Example

“Retrieve public Poshmark data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_poshmark_search` | Use Crawlora to access public Poshmark data. |

## Parameters

These are the exact generated input schemas.

### crawlora_poshmark_search
...
```json
{
  ...
  "properties": {
    ...
    "department": {
      "type": "string",
      "description": "department parameter."
    },
    "max_id": {
      "type": "string",
      "description": "max_id parameter."
    }
  }
}
```
