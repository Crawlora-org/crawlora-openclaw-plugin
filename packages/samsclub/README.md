# Samsclub by Crawlora

Public Samsclub data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Samsclub or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/samsclub --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-samsclub` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-samsclub --runtime --json
```

## Example

“Retrieve public Samsclub data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_samsclub_product` | Use Crawlora to access public Samsclub data. |

## Parameters

These are the exact generated input schemas.

### crawlora_samsclub_product
