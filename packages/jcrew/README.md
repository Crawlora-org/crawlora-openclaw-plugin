...
Public Jcrew data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Jcrew or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/jcrew --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-jcrew` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-jcrew --runtime --json
```

## Example

“Retrieve public Jcrew data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_jcrew_product` | Use Crawlora to access public Jcrew data. |

## Parameters

These are the exact generated input schemas.

### crawlora_jcrew_product
