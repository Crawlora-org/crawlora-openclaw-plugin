# Usptoppubs by Crawlora

Public Usptoppubs data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Usptoppubs or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/usptoppubs --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-usptoppubs` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-usptoppubs --runtime --json
```

## Example

“Retrieve public Usptoppubs data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_usptoppubs_search` | Use Crawlora to access public Usptoppubs data. |

## Parameters

These are the exact generated input schemas.

### crawlora_usptoppubs_search
...
```json
{
  ...
  "properties": {
    ...
    "databases": {
      "type": "string",
      "description": "databases parameter."
    },
    "num": {
      "type": "integer",
