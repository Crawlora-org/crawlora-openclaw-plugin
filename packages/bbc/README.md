...
Public Bbc data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Bbc or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/bbc --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-bbc` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-bbc --runtime --json
```

## Example

“Retrieve public Bbc data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_bbc_article` | Use Crawlora to access public Bbc data. |

## Parameters

These are the exact generated input schemas.

### crawlora_bbc_article
