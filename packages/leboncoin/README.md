# Leboncoin by Crawlora

Public Leboncoin data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Leboncoin or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/leboncoin --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-leboncoin` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-leboncoin --runtime --json
```

## Example

“Retrieve public Leboncoin data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_leboncoin_search` | Use Crawlora to access public Leboncoin data. |

## Parameters

These are the exact generated input schemas.

### crawlora_leboncoin_search
