...
Public Hotels data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Hotels or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/hotels --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-hotels` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-hotels --runtime --json
```

## Example

“Retrieve public Hotels data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_hotels_property` | Use Crawlora to access public Hotels data. |

## Parameters

These are the exact generated input schemas.

### crawlora_hotels_property
