# Ticketweb by Crawlora

Public Ticketweb data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Ticketweb or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/ticketweb --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-ticketweb` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-ticketweb --runtime --json
```

## Example

“Retrieve public Ticketweb data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_ticketweb_search` | Use Crawlora to access public Ticketweb data. |

## Parameters

These are the exact generated input schemas.

### crawlora_ticketweb_search
