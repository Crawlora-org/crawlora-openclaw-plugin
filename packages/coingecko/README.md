# CoinGecko by Crawlora

CoinGecko public market data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official CoinGecko or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/coingecko --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-coingecko` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-coingecko --runtime --json
```

## Example

“Get the top cryptocurrency markets priced in USD.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_coingecko_markets` | Fetch public cryptocurrency market rankings. |

## Parameters

These are the exact generated input schemas.

### crawlora_coingecko_markets
...
```json
{
  ...
  "properties": {
    "vs_currency": {
      "type": "string",
      "description": "Quote currency, such as usd or eur."
    },
    "page": {
      "type": "number",
