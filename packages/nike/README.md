...
Public Nike data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Nike or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/nike --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-nike` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-nike --runtime --json
```

## Example

“Retrieve public Nike data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_nike_product` | Use Crawlora to access public Nike data. |

## Parameters

These are the exact generated input schemas.

### crawlora_nike_product
...
```json
{
  ...
  "properties": {
    ...
    "style_color": {
      "type": "string",
      "description": "style_color parameter."
    }
  }
}
```
