# Draftkings by Crawlora

Public Draftkings data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Draftkings or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/draftkings --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-draftkings` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-draftkings --runtime --json
```

## Example

“Retrieve public Draftkings data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_draftkings_event` | Use Crawlora to access public Draftkings data. |

## Parameters

These are the exact generated input schemas.

### crawlora_draftkings_event
