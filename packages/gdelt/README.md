...
Public Gdelt data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Gdelt or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/gdelt --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-gdelt` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-gdelt --runtime --json
```

## Example

“Retrieve public Gdelt data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_gdelt_search` | Use Crawlora to access public Gdelt data. |

## Parameters

These are the exact generated input schemas.

### crawlora_gdelt_search
...
```json
{
  ...
  "properties": {
    ...
    "timespan": {
      "type": "string",
      "description": "timespan parameter."
    },
    "from": {
      "type": "string",
    ...
    "maxrecords": {
      "type": "integer",
      "description": "maxrecords parameter."
    }
  }
}
```
