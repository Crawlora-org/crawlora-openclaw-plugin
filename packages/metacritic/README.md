# Metacritic by Crawlora

Public Metacritic data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Metacritic or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/metacritic --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-metacritic` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-metacritic --runtime --json
```

## Example

“Retrieve public Metacritic data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_metacritic_game` | Use Crawlora to access public Metacritic data. |

## Parameters

These are the exact generated input schemas.

### crawlora_metacritic_game
