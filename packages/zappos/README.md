...
Public Zappos data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Zappos or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/zappos --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-zappos` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-zappos --runtime --json
```

## Example

“Retrieve public Zappos data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_zappos_search` | Use Crawlora to access public Zappos data. |

## Parameters

These are the exact generated input schemas.

### crawlora_zappos_search
