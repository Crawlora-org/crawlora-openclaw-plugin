# Producthunt by Crawlora

Public Producthunt data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Producthunt or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/producthunt --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-producthunt` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-producthunt --runtime --json
```

## Example

“Retrieve public Producthunt data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_producthunt_product` | Use Crawlora to access public Producthunt data. |

## Parameters

These are the exact generated input schemas.

### crawlora_producthunt_product
