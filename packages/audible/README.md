...
Public Audible data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Audible or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/audible --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-audible` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-audible --runtime --json
```

## Example

“Retrieve public Audible data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_audible_search` | Use Crawlora to access public Audible data. |

## Parameters

These are the exact generated input schemas.

### crawlora_audible_search
...
```json
{
  ...
  "properties": {
    ...
    "narrator": {
      "type": "string",
      "description": "narrator parameter."
    },
    "category_id": {
      "type": "string",
