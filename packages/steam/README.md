...
Steam public app data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Steam or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/steam --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-steam` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-steam --runtime --json
```

## Example

“Inspect the public Steam metadata for an app.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_steam_app` | Fetch public Steam app details. |
