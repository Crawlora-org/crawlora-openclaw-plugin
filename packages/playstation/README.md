# Playstation by Crawlora

Public Playstation data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Playstation or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/playstation --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-playstation` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-playstation --runtime --json
```

## Example

“Retrieve public Playstation data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_playstation_browse` | Use Crawlora to access public Playstation data. |

## Parameters

These are the exact generated input schemas.

### crawlora_playstation_browse
