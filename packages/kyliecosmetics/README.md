# Kyliecosmetics by Crawlora

Public Kyliecosmetics data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Kyliecosmetics or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/kyliecosmetics --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-kyliecosmetics` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-kyliecosmetics --runtime --json
```

## Example

“Retrieve public Kyliecosmetics data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_kyliecosmetics_product` | Use Crawlora to access public Kyliecosmetics data. |

## Parameters

These are the exact generated input schemas.

### crawlora_kyliecosmetics_product
