...
Public Grubhub data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Grubhub or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/grubhub --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-grubhub` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-grubhub --runtime --json
```

## Example

“Retrieve public Grubhub data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_grubhub_restaurant` | Use Crawlora to access public Grubhub data. |

## Parameters

These are the exact generated input schemas.

### crawlora_grubhub_restaurant
