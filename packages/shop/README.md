...
Public Shop data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Shop or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/shop --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-shop` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-shop --runtime --json
```

## Example

“Retrieve public Shop data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_shop_app_product` | Use Crawlora to access public Shop data. |

## Parameters

These are the exact generated input schemas.

### crawlora_shop_app_product
...
```json
{
  ...
  "properties": {
    ...
    "variant_id": {
      "type": "string",
      "description": "variant_id parameter."
    }
  }
}
```
