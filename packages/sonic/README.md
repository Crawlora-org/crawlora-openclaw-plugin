...
Public Sonic data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Sonic or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/sonic --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-sonic` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-sonic --runtime --json
```

## Example

“Retrieve public Sonic data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_sonic_nearby` | Use Crawlora to access public Sonic data. |

## Parameters

These are the exact generated input schemas.

### crawlora_sonic_nearby
