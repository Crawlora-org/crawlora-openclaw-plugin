# Jimmy Johns by Crawlora

Public Jimmy Johns data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Jimmy Johns or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/jimmy-johns --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-jimmy-johns` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-jimmy-johns --runtime --json
```

## Example

“Retrieve public Jimmy Johns data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_jimmy_johns_nearby` | Use Crawlora to access public Jimmy Johns data. |

## Parameters

These are the exact generated input schemas.

### crawlora_jimmy_johns_nearby
