# Foodpanda by Crawlora

Public Foodpanda data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Foodpanda or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/foodpanda --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-foodpanda` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-foodpanda --runtime --json
```

## Example

“Retrieve public Foodpanda data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_foodpanda_restaurant` | Use Crawlora to access public Foodpanda data. |

## Parameters

These are the exact generated input schemas.

### crawlora_foodpanda_restaurant
