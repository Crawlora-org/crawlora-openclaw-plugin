...
Public Goat data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Goat or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/goat --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-goat` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-goat --runtime --json
```

## Example

“Retrieve public Goat data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_goat_product` | Use Crawlora to access public Goat data. |

## Parameters

These are the exact generated input schemas.

### crawlora_goat_product
