...
Public Numbeo data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Numbeo or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/numbeo --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-numbeo` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-numbeo --runtime --json
```

## Example

“Retrieve public Numbeo data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_numbeo_cost_of_living_city` | Use Crawlora to access public Numbeo data. |

## Parameters

These are the exact generated input schemas.

### crawlora_numbeo_cost_of_living_city
