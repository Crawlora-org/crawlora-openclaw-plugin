...
Public Indeed data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Indeed or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/indeed --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-indeed` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-indeed --runtime --json
```

## Example

“Retrieve public Indeed data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_indeed_job` | Use Crawlora to access public Indeed data. |
