# Chick Fil A by Crawlora

Public Chick Fil A data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Chick Fil A or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/chick-fil-a --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-chick-fil-a` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-chick-fil-a --runtime --json
```

## Example

“Retrieve public Chick Fil A data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_chick_fil_a_content` | Use Crawlora to access public Chick Fil A data. |

## Parameters

These are the exact generated input schemas.

### crawlora_chick_fil_a_content
...
```json
{
  ...
  "properties": {
    ...
    "include_body": {
      "type": "boolean",
      "description": "include_body parameter."
    }
  }
}
```
