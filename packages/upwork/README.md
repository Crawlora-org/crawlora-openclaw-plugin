...
Public Upwork data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Upwork or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/upwork --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-upwork` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-upwork --runtime --json
```

## Example

“Retrieve public Upwork data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_upwork_search` | Use Crawlora to access public Upwork data. |

## Parameters

These are the exact generated input schemas.

### crawlora_upwork_search
