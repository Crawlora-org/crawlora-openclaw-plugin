# Guardian by Crawlora

Public Guardian data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Guardian or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/guardian --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-guardian` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-guardian --runtime --json
```

## Example

“Retrieve public Guardian data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_guardian_article` | Use Crawlora to access public Guardian data. |

## Parameters

These are the exact generated input schemas.

### crawlora_guardian_article
