...
Public Tmdb data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Tmdb or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/tmdb --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-tmdb` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-tmdb --runtime --json
```

## Example

“Retrieve public Tmdb data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_tmdb_movie` | Use Crawlora to access public Tmdb data. |
