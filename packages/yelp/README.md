...
Yelp public business search through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Yelp or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/yelp --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-yelp` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-yelp --runtime --json
```

## Example

“Find highly rated ramen restaurants in San Francisco.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_yelp_search` | Search public Yelp businesses. |

## Parameters

These are the exact generated input schemas.

### crawlora_yelp_search
...
```json
{
  ...
  "properties": {
    "term": {
      "type": "string",
      "description": "Business or service query."
    },
    "location": {
      "type": "string",
      "description": "City, address, or region."
    },
    "limit": {
      "type": "number",
