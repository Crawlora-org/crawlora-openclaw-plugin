# Pinterest by Crawlora

Pinterest public search through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Pinterest or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/pinterest --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-pinterest` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-pinterest --runtime --json
```

## Example

“Search Pinterest for minimalist kitchen ideas.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_pinterest_search` | Search public Pinterest pins and boards. |

## Parameters

These are the exact generated input schemas.

### crawlora_pinterest_search
