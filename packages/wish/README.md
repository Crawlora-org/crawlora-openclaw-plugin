...
Public Wish data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Wish or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/wish --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-wish` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-wish --runtime --json
```

## Example

“Retrieve public Wish data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_wish_search` | Use Crawlora to access public Wish data. |

## Parameters

These are the exact generated input schemas.

### crawlora_wish_search
