...
Public Yahoo data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Yahoo or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/yahoo --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-yahoo` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-yahoo --runtime --json
```

## Example

“Retrieve public Yahoo data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_yahoo_autos_home` | Use Crawlora to access public Yahoo data. |

## Parameters

These are the exact generated input schemas.

### crawlora_yahoo_autos_home
