...
Public Mlb data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Mlb or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/mlb --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-mlb` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-mlb --runtime --json
```

## Example

“Retrieve public Mlb data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_mlb_game` | Use Crawlora to access public Mlb data. |
