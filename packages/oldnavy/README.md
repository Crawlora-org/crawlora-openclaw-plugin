...
Public Oldnavy data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Oldnavy or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/oldnavy --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-oldnavy` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-oldnavy --runtime --json
```

## Example

“Retrieve public Oldnavy data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_oldnavy_product` | Use Crawlora to access public Oldnavy data. |

## Parameters

These are the exact generated input schemas.

### crawlora_oldnavy_product
