...
Public Kfc data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Kfc or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/kfc --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-kfc` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-kfc --runtime --json
```

## Example

“Retrieve public Kfc data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_kfc_nearby` | Use Crawlora to access public Kfc data. |
  ...
  "properties": {
    ...
    "occasion": {
      "anyOf": [
        ...
        {
          "type": "string",
          "const": "CATERING_CARRYOUT"
        }
      ],
      "description": "occasion parameter."
    },
    "max_results": {
      "type": "integer",
      "description": "max_results parameter."
    }
  }
}
```
