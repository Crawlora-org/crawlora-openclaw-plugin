# Colehaan by Crawlora

Public Colehaan data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Colehaan or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/colehaan --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-colehaan` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-colehaan --runtime --json
```

## Example

“Retrieve public Colehaan data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_colehaan_product` | Use Crawlora to access public Colehaan data. |

## Parameters

These are the exact generated input schemas.

### crawlora_colehaan_product
