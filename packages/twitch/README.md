...
Public Twitch data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Twitch or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/twitch --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-twitch` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-twitch --runtime --json
```

## Example

“Retrieve public Twitch data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_twitch_search` | Use Crawlora to access public Twitch data. |

## Parameters

These are the exact generated input schemas.

### crawlora_twitch_search
