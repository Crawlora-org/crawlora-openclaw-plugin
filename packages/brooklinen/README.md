# Brooklinen by Crawlora

Public Brooklinen data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Brooklinen or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/brooklinen --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-brooklinen` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-brooklinen --runtime --json
```

## Example

“Retrieve public Brooklinen data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_brooklinen_product` | Use Crawlora to access public Brooklinen data. |

## Parameters

These are the exact generated input schemas.

### crawlora_brooklinen_product
