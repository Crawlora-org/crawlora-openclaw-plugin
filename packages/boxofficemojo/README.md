# Boxofficemojo by Crawlora

Public Boxofficemojo data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Boxofficemojo or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/boxofficemojo --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-boxofficemojo` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-boxofficemojo --runtime --json
```

## Example

“Retrieve public Boxofficemojo data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_boxofficemojo_brand` | Use Crawlora to access public Boxofficemojo data. |

## Parameters

These are the exact generated input schemas.

### crawlora_boxofficemojo_brand
  ...
  "properties": {
    ...
    "sort": {
      "anyOf": [
        ...
        {
          "type": "string",
          "const": "maxNumTheaters"
        },
        {
          "type": "string",
          "const": "openingWeekendGross"
        },
        {
          "type": "string",
          "const": "openingNumTheaters"
        },
        {
          "type": "string",
          "const": "releaseDate"
