...
Public Bluesky data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Bluesky or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/bluesky --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-bluesky` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-bluesky --runtime --json
```

## Example

“Retrieve public Bluesky data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_bluesky_profile` | Use Crawlora to access public Bluesky data. |

## Parameters

These are the exact generated input schemas.

### crawlora_bluesky_profile
