...
Public X data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official X or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/x --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-x` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-x --runtime --json
```

## Example

“Retrieve public X data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_x_post` | Use Crawlora to access public X data. |
