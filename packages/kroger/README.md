...
Public Kroger data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Kroger or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/kroger --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-kroger` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-kroger --runtime --json
```

## Example

“Retrieve public Kroger data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_kroger_search` | Use Crawlora to access public Kroger data. |

## Parameters

These are the exact generated input schemas.

### crawlora_kroger_search
...
```json
{
  ...
  "properties": {
    ...
    "nutrition": {
      "type": "string",
      "description": "nutrition parameter."
    },
    "flavor": {
      "type": "string",
      "description": "flavor parameter."
    },
    "scent": {
      "type": "string",
      "description": "scent parameter."
    },
    "savings": {
      "type": "string",
      "description": "savings parameter."
    },
    "more_options": {
      "type": "string",
      "description": "more_options parameter."
    },
    "price_min": {
      "type": "number",
