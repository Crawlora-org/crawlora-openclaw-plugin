# Ubereats by Crawlora

Public Ubereats data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Ubereats or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/ubereats --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-ubereats` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-ubereats --runtime --json
```

## Example

“Retrieve public Ubereats data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_ubereats_search` | Use Crawlora to access public Ubereats data. |

## Parameters

These are the exact generated input schemas.

### crawlora_ubereats_search
