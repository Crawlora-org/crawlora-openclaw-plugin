# Pandamart by Crawlora

Public Pandamart data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Pandamart or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/pandamart --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-pandamart` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-pandamart --runtime --json
```

## Example

“Retrieve public Pandamart data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_pandamart_search` | Use Crawlora to access public Pandamart data. |

## Parameters

These are the exact generated input schemas.

### crawlora_pandamart_search
