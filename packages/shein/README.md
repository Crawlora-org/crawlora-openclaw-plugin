...
Public Shein data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Shein or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/shein --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-shein` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-shein --runtime --json
```

## Example

“Retrieve public Shein data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_shein_category_goods` | Use Crawlora to access public Shein data. |

## Parameters

These are the exact generated input schemas.

### crawlora_shein_category_goods
