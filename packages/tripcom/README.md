...
Public Tripcom data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Tripcom or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/tripcom --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-tripcom` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-tripcom --runtime --json
```

## Example

“Retrieve public Tripcom data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_tripcom_hotels_search` | Use Crawlora to access public Tripcom data. |

## Parameters

These are the exact generated input schemas.

### crawlora_tripcom_hotels_search
...
```json
{
  ...
  "properties": {
    "city_slug": {
      "type": "string",
      "description": "city_slug parameter."
    },
    "city_id": {
      "type": "string",
