...
Public Etsy data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Etsy or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/etsy --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-etsy` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-etsy --runtime --json
```

## Example

“Retrieve public Etsy data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_etsy_search` | Use Crawlora to access public Etsy data. |

## Parameters

These are the exact generated input schemas.

### crawlora_etsy_search
