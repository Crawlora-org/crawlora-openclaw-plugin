# Taco Bell by Crawlora

Public Taco Bell data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Taco Bell or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/taco-bell --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-taco-bell` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-taco-bell --runtime --json
```

## Example

“Retrieve public Taco Bell data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_taco_bell_menu` | Use Crawlora to access public Taco Bell data. |

## Parameters

These are the exact generated input schemas.

### crawlora_taco_bell_menu
