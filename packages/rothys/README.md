...
Public Rothys data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Rothys or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/rothys --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-rothys` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-rothys --runtime --json
```

## Example

“Retrieve public Rothys data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_rothys_product` | Use Crawlora to access public Rothys data. |

## Parameters

These are the exact generated input schemas.

### crawlora_rothys_product
