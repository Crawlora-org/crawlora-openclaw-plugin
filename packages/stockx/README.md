...
Public Stockx data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Stockx or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/stockx --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-stockx` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-stockx --runtime --json
```

## Example

“Retrieve public Stockx data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_stockx_product` | Use Crawlora to access public Stockx data. |

## Parameters

These are the exact generated input schemas.

### crawlora_stockx_product
