# Homedepot by Crawlora

Public Homedepot data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Homedepot or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/homedepot --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-homedepot` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-homedepot --runtime --json
```

## Example

“Retrieve public Homedepot data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_homedepot_categories` | Use Crawlora to access public Homedepot data. |

## Parameters

These are the exact generated input schemas.

### crawlora_homedepot_categories
