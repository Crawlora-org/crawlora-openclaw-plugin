...
Public Quince data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Quince or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/quince --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-quince` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-quince --runtime --json
```

## Example

“Retrieve public Quince data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_quince_product` | Use Crawlora to access public Quince data. |

## Parameters

These are the exact generated input schemas.

### crawlora_quince_product
