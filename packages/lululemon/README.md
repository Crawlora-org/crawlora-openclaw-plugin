# Lululemon by Crawlora

Public Lululemon data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Lululemon or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/lululemon --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-lululemon` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-lululemon --runtime --json
```

## Example

“Retrieve public Lululemon data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_lululemon_product` | Use Crawlora to access public Lululemon data. |

## Parameters

These are the exact generated input schemas.

### crawlora_lululemon_product
