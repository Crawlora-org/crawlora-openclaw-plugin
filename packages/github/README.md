...
GitHub repository search through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official GitHub or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/github --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-github` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-github --runtime --json
```

## Example

“Find popular open-source browser automation repositories.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_github_search_repositories` | Search public GitHub repositories. |

## Parameters

These are the exact generated input schemas.

### crawlora_github_search_repositories
