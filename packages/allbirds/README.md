# Allbirds by Crawlora

Public Allbirds data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Allbirds or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/allbirds --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-allbirds` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-allbirds --runtime --json
```

## Example

“Retrieve public Allbirds data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_allbirds_collection_products` | Use Crawlora to access public Allbirds data. |

## Parameters

These are the exact generated input schemas.

### crawlora_allbirds_collection_products
