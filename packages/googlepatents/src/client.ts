...
export async function callCrawlora(
  ...
  if (!apiKey) {
    throw new Error("Crawlora API key missing. Set plugins.entries.crawlora-googlepatents.config.apiKey or CRAWLORA_API_KEY.");
  }
  ...
  const client = new CrawloraClient({
    ...
    timeout: 60_000,
    retries: 0,
    userAgent: "crawlora-googlepatents/1.0.0",
  });
  try {
