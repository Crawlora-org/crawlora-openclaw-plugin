# Yahoo Finance by Crawlora

Yahoo Finance ticker quotes through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Yahoo Finance or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/yahoo-finance --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-yahoo-finance` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-yahoo-finance --runtime --json
```

## Example

“Get the latest available AAPL stock quote.”
...
### crawlora_yahoo_finance_ticker_quote
