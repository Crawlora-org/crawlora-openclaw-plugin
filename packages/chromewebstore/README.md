# Chromewebstore by Crawlora

Public Chromewebstore data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Chromewebstore or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/chromewebstore --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-chromewebstore` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-chromewebstore --runtime --json
```

## Example

“Retrieve public Chromewebstore data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_chromewebstore_search` | Use Crawlora to access public Chromewebstore data. |

## Parameters

These are the exact generated input schemas.

### crawlora_chromewebstore_search
