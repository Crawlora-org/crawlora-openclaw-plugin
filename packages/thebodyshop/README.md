# Thebodyshop by Crawlora

Public Thebodyshop data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Thebodyshop or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/thebodyshop --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-thebodyshop` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-thebodyshop --runtime --json
```

## Example

“Retrieve public Thebodyshop data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_thebodyshop_product` | Use Crawlora to access public Thebodyshop data. |

## Parameters

These are the exact generated input schemas.

### crawlora_thebodyshop_product
