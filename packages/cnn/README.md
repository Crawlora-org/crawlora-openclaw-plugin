...
Public Cnn data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Cnn or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/cnn --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-cnn` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-cnn --runtime --json
```

## Example

“Retrieve public Cnn data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_cnn_article` | Use Crawlora to access public Cnn data. |

## Parameters

These are the exact generated input schemas.

### crawlora_cnn_article
