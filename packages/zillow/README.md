...
Zillow public property search through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Zillow or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/zillow --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-zillow` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-zillow --runtime --json
```

## Example

“Browse homes for sale in Austin.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_zillow_search` | Search public Zillow property listings. |

## Parameters

These are the exact generated input schemas.

### crawlora_zillow_search
