# Gymshark by Crawlora

Public Gymshark data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Gymshark or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/gymshark --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-gymshark` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-gymshark --runtime --json
```

## Example

“Retrieve public Gymshark data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_gymshark_product` | Use Crawlora to access public Gymshark data. |

## Parameters

These are the exact generated input schemas.

### crawlora_gymshark_product
