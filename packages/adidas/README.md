...
Public Adidas data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Adidas or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/adidas --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-adidas` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-adidas --runtime --json
```

## Example

“Retrieve public Adidas data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_adidas_product` | Use Crawlora to access public Adidas data. |

## Parameters

These are the exact generated input schemas.

### crawlora_adidas_product
