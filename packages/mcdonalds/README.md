# Mcdonalds by Crawlora

Public Mcdonalds data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Mcdonalds or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/mcdonalds --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-mcdonalds` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-mcdonalds --runtime --json
```

## Example

“Retrieve public Mcdonalds data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_mcdonalds_item` | Use Crawlora to access public Mcdonalds data. |

## Parameters

These are the exact generated input schemas.

### crawlora_mcdonalds_item
