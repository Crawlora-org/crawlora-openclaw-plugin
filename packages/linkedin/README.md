# Linkedin by Crawlora

Public Linkedin data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Linkedin or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/linkedin --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-linkedin` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-linkedin --runtime --json
```

## Example

“Retrieve public Linkedin data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_linkedin_company` | Use Crawlora to access public Linkedin data. |

## Parameters

These are the exact generated input schemas.

### crawlora_linkedin_company
