# Sofascore by Crawlora

Public Sofascore data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Sofascore or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/sofascore --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-sofascore` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-sofascore --runtime --json
```

## Example

“Retrieve public Sofascore data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_sofascore_event` | Use Crawlora to access public Sofascore data. |

## Parameters

These are the exact generated input schemas.

### crawlora_sofascore_event
