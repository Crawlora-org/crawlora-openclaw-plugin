# Stevemadden by Crawlora

Public Stevemadden data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Stevemadden or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/stevemadden --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-stevemadden` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-stevemadden --runtime --json
```

## Example

“Retrieve public Stevemadden data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_stevemadden_product` | Use Crawlora to access public Stevemadden data. |

## Parameters

These are the exact generated input schemas.

### crawlora_stevemadden_product
