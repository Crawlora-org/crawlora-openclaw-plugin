...
Public Wendys data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Wendys or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/wendys --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-wendys` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-wendys --runtime --json
```

## Example

“Retrieve public Wendys data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_wendys_nearby` | Use Crawlora to access public Wendys data. |

## Parameters

These are the exact generated input schemas.

### crawlora_wendys_nearby
