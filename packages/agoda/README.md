...
Public Agoda data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Agoda or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/agoda --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-agoda` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-agoda --runtime --json
```

## Example

“Retrieve public Agoda data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_agoda_activities_search` | Use Crawlora to access public Agoda data. |

## Parameters

These are the exact generated input schemas.

### crawlora_agoda_activities_search
