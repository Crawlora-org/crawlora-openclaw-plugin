...
Public Boots data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Boots or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/boots --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-boots` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-boots --runtime --json
```

## Example

“Retrieve public Boots data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_boots_search` | Use Crawlora to access public Boots data. |

## Parameters

These are the exact generated input schemas.

### crawlora_boots_search
...
```json
{
  ...
  "properties": {
    ...
    "in_stock": {
      "type": "boolean",
      "description": "in_stock parameter."
    },
    "page": {
      "type": "integer",
