# Whataburger by Crawlora

Public Whataburger data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Whataburger or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/whataburger --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-whataburger` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-whataburger --runtime --json
```

## Example

“Retrieve public Whataburger data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_whataburger_store` | Use Crawlora to access public Whataburger data. |

## Parameters

These are the exact generated input schemas.

### crawlora_whataburger_store
