...
Public Kohls data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Kohls or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/kohls --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-kohls` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-kohls --runtime --json
```

## Example

“Retrieve public Kohls data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_kohls_stores` | Use Crawlora to access public Kohls data. |

## Parameters

These are the exact generated input schemas.

### crawlora_kohls_stores
