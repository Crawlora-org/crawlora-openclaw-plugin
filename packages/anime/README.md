...
Public Anime data through Crawlora. This package exposes 1 curated tools. It is an independent Crawlora integration, not an official Anime or OpenClaw product.
...
```sh
openclaw plugins install clawhub:@crawlora-org/anime --accept-capabilities
```
...
Restart the Gateway. If using plugin/tool allowlists, add `crawlora-anime` to the existing policy. Config takes precedence over the environment. Each platform has its own config entry; all can use the same Crawlora key.

```sh
openclaw plugins inspect crawlora-anime --runtime --json
```

## Example

“Retrieve public Anime data.”

## Tools

| Tool | Purpose |
| --- | --- |
| `crawlora_anime_character_search` | Use Crawlora to access public Anime data. |

## Parameters

These are the exact generated input schemas.

### crawlora_anime_character_search
