...
Give your OpenClaw agent 191 tools for live structured web data: search, product research, travel, media, markets, finance, SEC filings, and company hiring.
...
The 179 platform packages can run together, registering 191 distinct tools. The catalog includes focused adapters for major public families and is generated from the published Crawlora SDK operation metadata. Disable the overlapping starter first with `openclaw plugins disable crawlora`. The platform packages refuse registration while the starter's config entry is enabled, preventing duplicate tool registrations. Do not re-enable both at once.
