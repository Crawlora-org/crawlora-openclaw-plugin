...
The ten platform packages can run together, registering 21 distinct tools. Disable the overlapping starter first with `openclaw plugins disable crawlora`. The platform packages refuse registration while the starter's config entry is enabled, preventing duplicate tool registrations. Do not re-enable both at once.
